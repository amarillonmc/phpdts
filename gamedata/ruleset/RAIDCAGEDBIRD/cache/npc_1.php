<?php

	if(!defined('IN_GAME')) exit('Access Denied');

	$npcinit = array
	(
		'name' => '',	'pass' => 'bra', 'gd' => 'm',	'icon' => 0,	'club' => 0,
		'mhp' => 0,	'msp' => 0,	'att' => 0,	'def' => 0,	'pls' => 0,	'lvl' => 0,
		'money' => 0,	'inf' => '',	'rage' => 0,	'pose' => 0,	'tactic' => 0,
		'killnum' => 0,	'state' => 1,	'teamID' => '',	'teamPass' => '','bid' => 0,
		'horizon' => 0, 'clbpara' => Array(),
		'wp' => 0, 'wk' => 0, 'wc' => 0, 'wg' => 0, 'wd' => 0, 'wf' => 0, 'skills' => 0, 'rp' => 0,
		'wep' => '',	'wepk' => '',	'wepe' => 0,	'weps' => 0,	'wepsk' => '',
		'arb' => '',	'arbk' => '',	'arbe' => 0,	'arbs' => 0,	'arbsk' => '',
		'arh' => '',	'arhk' => '',	'arhe' => 0,	'arhs' => 0,	'arhsk' => '',
		'arf' => '',	'arfk' => '',	'arfe' => 0,	'arfs' => 0,	'arfsk' => '',
		'ara' => '',	'arak' => '',	'arae' => 0,	'aras' => 0,	'arask' => '',
		'art' => '',	'artk' => '',	'arte' => 0,	'arts' => 0,	'artsk' => '',
		'itm0' => '',	'itmk0' => '',	'itme0' => 0,	'itms0' => 0,	'itmsk0' => '',
		'itm1' => '',	'itmk1' => '',	'itme1' => 0,	'itms1' => 0,	'itmsk1' => '',
		'itm2' => '',	'itmk2' => '',	'itme2' => 0,	'itms2' => 0,	'itmsk2' => '',
		'itm3' => '',	'itmk3' => '',	'itme3' => 0,	'itms3' => 0,	'itmsk3' => '',
		'itm4' => '',	'itmk4' => '',	'itme4' => 0,	'itms4' => 0,	'itmsk4' => '',
		'itm5' => '',	'itmk5' => '',	'itme5' => 0,	'itms5' => 0,	'itmsk5' => '',
		'itm6' => '',	'itmk6' => '',	'itme6' => 0,	'itms6' => 0,	'itmsk6' => '',
	);
	$npcinfo = array
	(
	//1 = 红暮
	1 => array
		(
		'mode' => 1,
		'num' => 1,
		'pass' => 'bra',
		'club' => 4,
		'clubskill' => Array(),
		'clubskillpara' => Array
		(
			'c4_stable' => Array(
				'lvl' => 5,
				'costcount' => 14,
			),
			'c4_break' => Array(
				'lvl' => 3,
				'costcount' => 19,
			),
			'c4_roar' => Array(
				'active' => 1,
			),
		),
		'skills' => 66,
		'bid' => 0,
		'inf' => '',
		'rage' => 0,
		'pose'=> 2,
		'tactic' => 3,
		'killnum' => 0,
		'teamID' => '',
		'teamPass' => '',
		'gd' => 'f',
		'pls' => 0,
		'mhp' => 7500,
		'msp' => 4000,
		'att' => 750,
		'def' => 750,
		'lvl' => 75,
		'skill' => 777,
		'wg' => 1420,
		'money' => 9999,
		'arb' => '奥术盔甲B - 炎',
		'arbk' => 'DB',
		'arbe' => 3000,
		'arbs' => 3000,
		'arbsk' => 'Ab',
		'arh' => '奥术盔甲H - 炎',
		'arhk' => 'DH',
		'arhe' => 1444,
		'arhs' => 3000,
		'arf' => '奥术盔甲F - 炎',
		'arfk' => 'DF',
		'arfe' => 1444,
		'arfs' => 3000,
		'ara' => '奥术盔甲A - 炎',
		'arak' => 'DA',
		'arae' => 1444,
		'aras' => 3000,
		'arask' => '',
		'art' => '龙虎旗帜',
		'artk' => 'A',
		'arte' => 1000,
		'arts' => 1000,
		'artsk' => 'H',
		'sub' => array
		(
			0 => array
			(
			'name' => '红暮',
			'icon' => 7,
			'wep' => '燃素加农炮『爆炎』MK-II',
			'wepk' => 'WG',
			'wepe' => 1280,
			'weps' => 9999,
			'wepsk' => 'rfn',
			),
		),
	),

	14 => array(
		'mode' => 1,
        'num' => 3,
		'pass' => 'bra',
		'club' => 99,
		'bid' => 0,
		'inf' => '',
		'state' => 1,
		'rage' => 5,
		'pose'=> 0,
		'tactic' => 0,
		'killnum' => 0,
		'rp' => 800,
		'teamID' => '循环',
		'teamPass' => 'L1g2t2D2k',
		'gd' => 'f',
		'pls' => 99,
		'mhp' => 2800,
		'msp' => 400,
		'att' => 75,
		'def' => 75,
		'lvl' => 20,
		'skill' => 25,
		'money' => 3,
		'sub' => array
		(
			0 => array
			(
			'name' => '讲解员 梦美',
			'icon' => 61,
			'num' => 1,
			'wep' => '垃圾花束',
			'wepk' => 'WP',
			'wepe' => 175,
			'weps' => 36,
			'wepsk' => 'c',
			'arb' => '接待员制服',
			'arbk' => 'DB',
			'arbe' => 75,
			'arbs' => 50,
			'arbsk' => 'a',
			'arh' => '橙色信号缎带',
			'arhk' => 'DH',
			'arhe' => 640,
			'arhs' => 50,
			'arf' => '蓝色信号缎带',
			'arfk' => 'DF',
			'arfe' => 640,
			'arfs' => 50,
			'ara' => '绿色信号缎带',
			'arak' => 'DA',
			'arae' => 640,
			'aras' => 50,
			'art' => '星空之愿',
			'artk' => 'A',
			'arte' => 1,
			'arts' => 1,
			'artsk' => 'H',
			),
			1 => array
			(
			'name' => '喧哗少女 叶留佳',
			'icon' => 63,
			'num' => 1,
			'wep' => '七色玻璃珠',
			'wepk' => 'WC',
			'wepe' => 160,
			'weps' => 36,
			'wepsk' => 'ec',
			'arb' => 'RF高校校服',
			'arbk' => 'DB',
			'arbe' => 75,
			'arbs' => 50,
			'arbsk' => 'a',
			'arh' => '粉红双球发饰',
			'arhk' => 'DH',
			'arhe' => 640,
			'arhs' => 50,
			'arf' => '女式皮鞋',
			'arfk' => 'DF',
			'arfe' => 640,
			'arfs' => 50,
			'ara' => '奇怪的袋子',
			'arak' => 'DA',
			'arae' => 640,
			'aras' => 50,
			'art' => '友情之愿',
			'artk' => 'A',
			'arte' => 1,
			'arts' => 1,
			'artsk' => 'H',
			),
			2 => array
			(
			'name' => '风纪委员 静流',
			'icon' => 65,
			'num' => 1,
			'wep' => '银白口哨',
			'wepk' => 'WF',
			'wepe' => 175,
			'weps' => 200,
			'wepsk' => 'c',
			'arb' => '风祭学院校服',
			'arbk' => 'DB',
			'arbe' => 75,
			'arbs' => 50,
			'arbsk' => '',
			'arh' => '白色眼罩',
			'arhk' => 'DH',
			'arhe' => 640,
			'arhs' => 50,
			'arf' => '女式运动鞋',
			'arfk' => 'DF',
			'arfe' => 640,
			'arfs' => 50,
			'ara' => 'Mp3播放器',
			'arak' => 'DA',
			'arae' => 640,
			'aras' => 50,
			'art' => '平和之愿',
			'artk' => 'A',
			'arte' => 1,
			'arts' => 1,
			'artsk' => 'H',
			),
		),
	),
	15 => array(
		'mode' => 1,
		'num' => 0,
		'pass' => 'bra',
		'club' => 98,
		'bid' => 0,
		'inf' => '',
		'state' => 1,
		'rage' => 5,
		'pose'=> 0,
		'tactic' => 4,
		'killnum' => 0,
		'teamID' => '',
		'teampsss' => '',
		'gd' => 'f',
		'pls' => 99,
		'mhp' => 900,
		'msp' => 1600,
		'att' => 900,
		'def' => 400,
		'lvl' => 42,
		'skill' => 50,
		'wp' => 300, 'wk' => 555, 'wc' => 684, 'wg' => 880, 'wd' => 360, 'wf' => 80,
		'money' => 123,
		'arb' => '气息遮蔽装置',
		'arbk' => 'DB',
		'arbe' => 400,
		'arbs' => 220,
		'arbsk' => 'Aa',
		'arh' => '身份遮蔽装置',
		'arhk' => 'DH',
		'arhe' => 360,
		'arhs' => 220,
		'arhsk' => 'm',
		'arf' => '反制遮蔽装置',
		'arfk' => 'DF',
		'arfe' => 360,
		'arfs' => 220,
		'arfsk' => 'b',
		'ara' => '罪业遮蔽装置',
		'arak' => 'DA',
		'arae' => 360,
		'aras' => 220,
		'arask' => 'H',
		'art' => '红色按钮',
		'artk' => 'A',
		'arte' => 1,
		'arts' => 1,
		'artsk' => 'Z',
		'sub' => array
		(
			0 => array
			(
			'name' => '【SANMA_TK】',
			'icon' => 69,
			'wep' => '元素枪刃『Glider』',
			'wepk' => 'WGK',
			'wepe' => 400,
			'weps' => 9999,
			'wepsk' => 'peNn',
			'itm1' => '断罪之光刃',
			'itmk1' => 'WK',
			'itme1' => 300,
			'itms1' => 500,
			'itmsk1' => 'dNne',
			'itm2' => '破灭之天雷',
			'itmk2' => 'WC',
			'itme2' => 300,
			'itms2' => 500,
			'itmsk2' => 'wNne',
			'itm3' => '无尽之洪流',
			'itmk3' => 'WP',
			'itme3' => 300,
			'itms3' => 500,
			'itmsk3' => 'uNne',
			'itm4' => '黄金秋刀鱼',
			'itmk4' => 'HB',
			'itme4' => 9999,
			'itms4' => '∞',
			'itmsk4' => 'Z',
			'itm5' => '限定型-MSM-04 アッガイ',
			'itmk5' => 'X',
			'itme5' => 9999,
			'itms5' => 1,
			'itmsk5' => 'Z',
			'itm6' => '✦种火聚集装置✦',
			'itmk6' => 'Y',
			'itme6' => 1,
			'itms6' => 1,
			),
		),
	),

	19 => array(
		'mode' => 1,
		'num' => 0,
		'pass' => 'bra',
		'bid' => 0,
		'inf' => '',
		'state' => 1,
		'rage' => 5,
		'pose'=> 0,
		'tactic' => 4,
		'killnum' => 0,
		'teamID' => '',
		'teampsss' => '',
		'gd' => 'f',
		'pls' => 0,
		'lvl' => 251,
		'skill' => 1888,
		'wp' => 1888, 'wk' => 1888, 'wc' => 1888, 'wg' => 1888, 'wd' => 1888, 'wf' => 1888,
		'money' => 1,
		'arb' => '冰炎盔甲B',
		'arbk' => 'DB',
		'arbe' => 4000,
		'arbs' => 3300,
		'arbsk' => 'Aa',
		'arh' => '冰炎盔甲H',
		'arhk' => 'DH',
		'arhe' => 4000,
		'arhs' => 3300,
		'arhsk' => 'm',
		'arf' => '冰炎盔甲F',
		'arfk' => 'DF',
		'arfe' => 4000,
		'arfs' => 3300,
		'arfsk' => 'b',
		'ara' => '冰炎盔甲A',
		'arak' => 'DA',
		'arae' => 4000,
		'aras' => 3300,
		'arask' => 'H',
		'art' => '龙虎标记',
		'artk' => 'A',
		'arte' => 1,
		'arts' => 1,
		'artsk' => 'Z',
		'sub' => array
		(
			0 => array
			(
			'name' => '红暮',
			'club' => 98,
			'mhp' => 20800,
			'msp' => 2800,
			'att' => 1100,
			'def' => 1500,
			'icon' => 7,
			'wep' => '喷气式红杀重铁剑',
			'wepk' => 'WK',
			'wepe' => 1107,
			'weps' => 9999,
			'wepsk' => 'rfnv',
			'itm1' => '绯红记忆',
			'itmk1' => 'WF',
			'itme1' => 1107,
			'itms1' => 9999,
			'itmsk1' => 'rfnv',
			'itm2' => '血色强袭',
			'itmk2' => 'WG',
			'itme2' => 1107,
			'itms2' => 9999,
			'itmsk2' => 'rfnv',
			'itm3' => '狮虎丝带',
			'itmk3' => 'WD',
			'itme3' => 1107,
			'itms3' => 9999,
			'itmsk3' => 'rfnv',
			'itm4' => '落樱巨锤',
			'itmk4' => 'WP',
			'itme4' => 1107,
			'itms4' => 1999,
			'itmsk4' => 'rfnv',
			'itm5' => '八八连流星浮游炮',
			'itmk5' => 'WC',
			'itme5' => 1107,
			'itms5' => 9999,
			'itmsk5' => 'rfnv',
			),
			1 => array
			(
			'name' => '蓝凝',
			'club' => 19,
			'mhp' => 9999998,
			'msp' => 1600,
			'att' => 199900,
			'def' => 2800,
			'icon' => 52,
			'wep' => '『AZURE RONDO · AQUAMARINE』',
			'wepk' => 'WFK',
			'wepe' => 1999,
			'weps' => 9999,
			'wepsk' => 'rknv',
			'itm1' => '♪臻蓝之愿♪',
			'itmk1' => 'T',
			'itme1' => 1001,
			'itms1' => 9999,
			'itmsk1' => 'rknv',
			'itm2' => '♫钴蓝之灵♫',
			'itmk2' => 'T',
			'itme2' => 1001,
			'itms2' => 9999,
			'itmsk2' => 'rknv',
			'itm3' => '❀矢车菊的回忆❀',
			'itmk3' => 'T',
			'itme3' => 10001,
			'itms3' => 9999,
			'itmsk3' => 'rknv',
			),
		),
	),

	20 => array
		(
		'mode' => 2,
		'num' => 10,
		'pass' => 'gbauibg2',
		'club' => 0,
		'bid' => 0,
		'inf' => '',
		'rage' => 88,
		'pose'=> 1,
		'tactic' => 3,
		'killnum' => 0,
		'rp' => 1200,
		'teamID' => '',
		'teamPass' => '',
		'gd' => 'r',
		'pls' => 34,
		'mhp' => 3000,
		'msp' => 400,
		'att' => 200,
		'def' => 200,
		'lvl' => 40,
		'skill' => 300,
		'money' => 800,
		'arb' => '英雄战甲B',
		'arbk' => 'DB',
		'arbe' => 1000,
		'arbs' => 300,
		'arbsk' => 'A',
		'arh' => '英雄战甲H',
		'arhk' => 'DH',
		'arhe' => 600,
		'arhs' => 300,
		'arhsk' => 'DF',
		'arf' => '英雄战甲F',
		'arfk' => 'DF',
		'arfe' => 500,
		'arfs' => 300,
		'arfsk' => 'a',
		'ara' => '英雄战甲A',
		'arak' => 'DA',
		'arae' => 600,
		'aras' => 300,
		'arask' => 'H',
		'art' => '英雄之力',
		'artk' => 'A',
		'arte' => 9,
		'arts' => 9,
		'artsk' => 'c',
//		'itm5' => '好人卡',
//		'itmk5' => 'Y',
//		'itme5' => 1,
//		'itms5' => 88,
		'sub' => array
		(
			0 => array
			(
			'name' => '霜火协奏曲',
			'icon' => 10,
			'club' => 6,
			'wep' => '『看吧，你的死兆星正在天上闪耀！』',
			'wepk' => 'WC',
			'wepe' => 333,
			'weps' => 600,
			'wepsk' => 'w',
			),
			1 => array
			(
			'name' => '脸骑士-菜包',
			'club' => 98,
			'icon' => 10,
			'wep' => '脸',
			'wepk' => 'WP',
			'wepe' => 300,
			'weps' => 600,
			'wepsk' => 'N',
			),
			2 => array
			(
			'name' => '枪毙的某神',
			'icon' => 10,
			'club' => 3,
			'wep' => '《小黄的时间球》',
			'wepk' => 'WC',
			'wepe' => 180,
			'weps' => 600,
			'wepsk' => 'r',
			),
			3 => array
			(
			'name' => '我是高达',
			'icon' => 10,
			'club' => 8,
			'wep' => '毒性凸眼鱼',
			'wepk' => 'WF',
			'wepe' => 200,
			'weps' => 600,
			'wepsk' => 'pd',
			),
			4 => array
			(
			'name' => '一旦接受这种设定',
			'icon' => 10,
			'club' => 10,
			'wep' => '『一瞬千击』',
			'wepk' => 'WP',
			'wepe' => 190,
			'weps' => 600,
			'wepsk' => 'r',
			),
			5 => array
			(
			'name' => '一方通行',
			'icon' => 10,
			'club' => 9,
			'wep' => '【矢量操作】',
			'wepk' => 'WG',
			'wepe' => 300,
			'weps' => 600,
			'wepsk' => 'N',
			),
			6 => array
			(
			'name' => '晓魂之歌',
			'icon' => 10,
			'club' => 2,
			'wep' => '锋利的电气毒性晓魂之歌-改[+4]',
			'wepk' => 'WK',
			'wepe' => 300,
			'weps' => 600,
			'wepsk' => 'pew',
			),
			7 => array
			(
			'name' => '卡',
			'icon' => 10,
			'club' => 12,
			'wep' => '【不屈的意志】',
			'wepk' => 'WF',
			'wepe' => 140,
			'weps' => 600,
			'wepsk' => 'r',
			),
			8 => array
			(
			'name' => 'Exocet',
			'icon' => 10,
			'club' => 9,
			'wep' => '『微型火箭加速噴射單輪車』',
			'wepk' => 'WP',
			'wepe' => 110,
			'weps' => 600,
			'wepsk' => 'ru',
			),
			9 => array
			(
			'name' => '中西醫',
			'icon' => 10,
			'club' => 1,
			'wep' => '《衷中參西錄》',
			'wepk' => 'WP',
			'wepe' => 250,
			'weps' => 600,
			'wepsk' => 'p',
			),
			10 => array
			(
			'name' => '黑猫',
			'icon' => 10,
			'club' => 6,
			'wep' => '阔剑地雷吸附器',
			'wepk' => 'WP',
			'wepe' => 400,
			'weps' => 600,
			'wepsk' => 'Mm',
			),
			11 => array
			(
			'name' => '水羊',
			'icon' => 10,
			'club' => 2,
			'wep' => '■Darthnote■',
			'wepk' => 'WK',
			'wepe' => 400,
			'weps' => 600,
			'wepsk' => 'c',
			),
			12 => array
			(
			'name' => '上帝的左手',
			'icon' => 10,
			'club' => 2,
			'wep' => '胡来的左手',
			'wepk' => 'WK',
			'wepe' => 270,
			'weps' => 600,
			'wepsk' => 'd',
			),
			13 => array
			(
			'name' => '半人半灵半吊子',
			'icon' => 10,
			'club' => 98,
			'wep' => '楼观剑',
			'wepk' => 'WK',
			'wepe' => 200,
			'weps' => 600,
			'wepsk' => 'd',
			'itm1' => '白楼剑',
			'itmk1' => 'WK',
			'itme1' => 130,
			'itms1' => 600,
			'itmsk1' => 'r',
			),
			14 => array
			(
			'name' => '死亡荆棘',
			'icon' => 10,
			'club' => 7,
			'wep' => '【荆棘式电子地雷】',
			'wepk' => 'WK',
			'wepe' => 210,
			'weps' => 600,
			'wepsk' => 'd',
			),
			15 => array
			(
			'name' => '超魔理沙-电子',
			'icon' => 10,
			'club' => 12,
			'wep' => '【荆棘式电子地雷】',
			'wepk' => 'WP',
			'wepe' => 240,
			'weps' => 600,
			'wepsk' => 'd',
			),
			16 => array
			(
			'name' => '东方地雷殿',
			'icon' => 10,
			'club' => 7,
			'wep' => '【荆棘式电子地雷】',
			'wepk' => 'WD',
			'wepe' => 210,
			'weps' => 600,
			'wepsk' => 'd',
			),
			17 => array
			(
			'name' => 'fossil',
			'icon' => 10,
			'club' => 1,
			'wep' => '大钉棍棒',
			'wepk' => 'WP',
			'wepe' => 250,
			'weps' => 600,
			'wepsk' => 'Nn',
			),
			18 => array
			(
			'name' => '流风之念',
			'icon' => 10,
			'club' => 4,
			'wep' => '连击烧输尿管~☆',
			'wepk' => 'WG',
			'wepe' => 150,
			'weps' => 600,
			'wepsk' => 'ru',
			),
			19 => array
			(
			'name' => 'MESSIAH',
			'icon' => 10,
			'club' => 4,
			'wep' => '本格的嘴炮',
			'wepk' => 'WG',
			'wepe' => 240,
			'weps' => 600,
			'wepsk' => 'd',
			),
			20 => array
			(
			'name' => '帕秋莉诺蕾姬',
			'icon' => 10,
			'club' => 9,
			'wep' => '水&火符「Phlogistic Rain」',
			'wepk' => 'WF',
			'wepe' => 260,
			'weps' => 600,
			'wepsk' => 'ui',
			),
			21 => array
			(
			'name' => '坂田铜时',
			'icon' => 10,
			'club' => 1,
			'wep' => '无毁的受王拳',
			'wepk' => 'WP',
			'wepe' => 320,
			'weps' => 600,
			),
			22 => array
			(
			'name' => '耶和华',
			'icon' => 10,
			'club' => 15,
			'wep' => 'L5爆发！',
			'wepk' => 'WF',
			'wepe' => 250,
			'weps' => 600,
			'wepsk' => 'd',
			),
			23 => array
			(
			'name' => '闹球肾',
			'icon' => 10,
			'club' => 1,
			'wep' => '一个半角符号',
			'wepk' => 'WF',
			'wepe' => 150,
			'weps' => 600,
			'wepsk' => 'rd',
			),
			24 => array
			(
			'name' => '初音ミク',
			'icon' => 10,
			'club' => 4,
			'wep' => '「Falchion Rider」模样的杏仁豆腐',
			'wepk' => 'WG',
			'wepe' => 190,
			'weps' => 600,
			'wepsk' => 'r',
			'arb' => '英雄战甲B模样的杏仁豆腐',
			'arh' => '英雄战甲H模样的杏仁豆腐',
			'arf' => '英雄战甲F模样的杏仁豆腐',
			'ara' => '英雄战甲A模样的杏仁豆腐',
			'art' => '英雄之力模样的杏仁豆腐',
			),
			25 => array
			(
			'name' => 'Abyss混沌',
			'icon' => 10,
			'club' => 1,
			'wep' => '混乱邪恶之塔',
			'wepk' => 'WF',
			'wepe' => 1,
			'weps' => 1,
			'wepsk' => 'r',
			),
			26 => array
			(
			'name' => '飞雪大大',
			'icon' => 10,
			'club' => 9,
			'wep' => '魔王の剑',
			'wepk' => 'WK',
			'wepe' => 300,
			'weps' => 300,
			'wepsk' => 'u',
			),
			27 => array
			(
			'name' => '微笑的疯子',
			'icon' => 10,
			'club' => 1,
			'wep' => '把妹の手',
			'wepk' => 'WP',
			'wepe' => 100,
			'weps' => 600,
			'wepsk' => 'Nnr',
			),
			28 => array
			(
			'name' => '挂机中的灰尘',
			'icon' => 10,
			'club' => 9,
			'wep' => '★挂机の萌力★',
			'wepk' => 'WF',
			'wepe' => 250,
			'weps' => 600,
			'wepsk' => 'ni',
			),
			29 => array
			(
			'name' => '西园寺世界酱',
			'icon' => 10,
			'club' => 5,
			'wep' => '节操炸弹G',
			'wepk' => 'WD',
			'wepe' => 300,
			'weps' => 600,
			'wepsk' => 'id',
			),
			30 => array
			(
			'name' => '索非亚小傻鸟',
			'icon' => 10,
			'club' => 8,
			'wep' => 'COCO☆酱',
			'wepk' => 'WC',
			'wepe' => 280,
			'weps' => 600,
			'wepsk' => 'lu',
			),
			31 => array
			(
			'name' => '浮云_小坏',
			'icon' => 10,
			'club' => 9,
			'wep' => '向日葵妖精',
			'wepk' => 'WG',
			'wepe' => 251,
			'weps' => 600,
			'wepsk' => 'ud',
			'itm1' => 'YES♂SIR',
			'itmk1' => 'Z',
			'itme1' => 1,
			'itms1' => 1,
			),
			32 => array
			(
			'name' => '人形蚂蚁α',
			'icon' => 10,
			'club' => 9,
			'wep' => '纸条■■■■',
			'wepk' => 'WC',
			'wepe' => 280,
			'weps' => 600,
			'wepsk' => 'Ne',
			),
			33 => array
			(
			'name' => 'tabris',
			'icon' => 10,
			'club' => 12,
			'wep' => '十二试炼',
			'wepk' => 'WC',
			'wepe' => 400,
			'weps' => 600,
			'wepsk' => 'r',
			),
		),
	),

	21 => array
		(
		'mode' => 2,
		'num' => 5,
		'pass' => 'wei42bg',
		'club' => 0,
		'bid' => 0,
		'inf' => '',
		'rage' => 250,
		'pose'=> 2,
		'tactic' => 3,
		'killnum' => 0,
		'teamID' => '',
		'teamPass' => '',
		'gd' => 'r',
		'pls' => 34,
		'mhp' => 10000,
		'msp' => 800,
		'att' => 3000,
		'def' => 3000,
		'lvl' => 65,
		'skill' => 600,
		'money' => 40000,
		'arb' => '武神战甲B',
		'arbk' => 'DB',
		'arbe' => 3000,
		'arbs' => 300,
		'arbsk' => 'A',
		'arh' => '武神战甲H',
		'arhk' => 'DH',
		'arhe' => 2000,
		'arhs' => 300,
		'arhsk' => 'b',
		'arhsk' => 'DF',
		'arf' => '武神战甲F',
		'arfk' => 'DF',
		'arfe' => 2000,
		'arfs' => 300,
		'artsk' => 'a',
		'ara' => '武神战甲A',
		'arak' => 'DA',
		'arae' => 2000,
		'aras' => 300,
		'arask' => 'H',
		'art' => '武神之魂',
		'artk' => 'A',
		'arte' => 9,
		'arts' => 9,
		'artsk' => 'ch',
//		'itm5' => '好人卡',
//		'itmk5' => 'Y',
//		'itme5' => 1,
//		'itms5' => 88,
		'sub' => array
		(
			0 => array
			(
			'name' => '虚子',
			'gd' => 'f',
			'icon' => 101,
			'club' => 10,
			'clubskill' => Array(
				'c4_stable','c4_break',
			),
			'clubskillpara' => Array
			(
				'c4_stable' => Array(
					'lvl' => 3,
				),
				'c4_break' => Array(
					'lvl' => 3,
				),
				'c10_insight' => Array(
					'lvl' => 4,
				),
			),
			'mhp' => 4932,
			'att' => 5120,
			'skill' => 1500,
			'wep' => '燕返262',
			'wepk' => 'WG',
			'wepe' => 3000,
			'weps' => 3000,
			'wepsk' => 'XNnrd',
			'arb' => '空军制服',
			'arbk' => 'DB',
			'arbe' => 2333,
			'arbs' => 300,
			'arbsk' => 'ZA',
			'arh' => '飞行员头盔',
			'arhk' => 'DH',
			'arhe' => 2333,
			'arhs' => 300,
			'arhsk' => 'ZB',
			'arf' => '空军马裤',
			'arfk' => 'DF',
			'arfe' => 2333,
			'arfs' => 300,
			'arfsk' => 'Za',
			'ara' => '老式电子表',
			'arak' => 'DA',
			'arae' => 2333,
			'aras' => 300,
			'arask' => 'ZH',
			'art' => '耶律哥的喇叭',
			'artk' => 'A',
			'arte' => 1,
			'arts' => 1,
			'artsk' => 'Zch',
			'itm1' => '『寻星者』',
			'itmk1' => 'WK',
			'itme1' => 90,
			'itms1' => 350,
			'itmsk1' => 'Nnrd',
			'itm2' => '超重型炮塔列车 古斯塔夫最大炮',
			'itmk2' => 'WG',
			'itme2' => 3000,
			'itms2' => 3000,
			'itmsk2' => 'Nnrd',
			'itm2' => '《董子语录》',
			'itmk2' => 'VV',
			'itme2' => 153,
			'itms2' => 1,
			'itmsk2' => 'Z',
			),
			1 => array
			(
			'name' => '水月',
			'gd' => 'f',
			'icon' => 99,
			'club' => 98,
			'mode' => 2,
			'mhp' => 12000,
			'skill' => 800,
			'wep' => '「饭纲权现降临」',
			'wepk' => 'WC',
			'wepe' => 1,
			'weps' => 9999,
			'wepsk' => 'Bbx',
			'arb' => 'Reality marble',
			'arbk' => 'DB',
			'arbe' => 3000,
			'arbs' => 500,
			'arbsk' => 'Ax',
			'arh' => 'Torah',
			'arhk' => 'DH',
			'arhe' => 3000,
			'arhs' => 300,
			'arhsk' => 'bx',
			'arf' => 'Nevi’im',
			'arfk' => 'DF',
			'arfe' => 3000,
			'arfs' => 300,
			'arfsk' => 'ax',
			'ara' => 'Fantasm',
			'arak' => 'DA',
			'arae' => 3000,
			'aras' => 300,
			'arask' => 'Hx',
			'art' => '《Dead Sea Scrolls》',
			'artk' => 'Ah',
			'arte' => 3000,
			'arts' => 233,
			'artsk' => 'cx',
			'itm1' => 'EX火&金符『St. Elmo Pillar』',
			'itmk1' => 'WF',
			'itme1' => 700,
			'itms1' => '∞',
			'itmsk1' => 'nrdf',
			'itm2' => 'Barrett M95',
			'itmk2' => 'WG',
			'itme2' => 500,
			'itms2' => 500,
			'itmsk2' => 'rnfS',
			'itm3' => '圣光啊!你有看到那个敌人吗！',
			'itmk3' => 'Z',
			'itme3' => 1,
			'itms3' => 1,
			),
			2 => array
			(
			'name' => '黑色奪魂曲',
			'gd' => 'f',
			'icon' => 104,
			'club' => 99,
			'mhp' => 10000,
			'att' => 100,
			'def' => 400,
			'rage' => 10,
			'lvl' => 20,
			'skill' => 100,
			'wep' => '超⑨武神斩',
			'wepk' => 'WK',
			'wepe' => 200,
			'weps' => 100,
			'wepsk' => 'z',
			'arb' => '黑曲装备B',
			'arbk' => 'DB',
			'arbe' => 400,
			'arbs' => 300,
			'arbsk' => 'zh',
			'arh' => '黑曲装备H',
			'arhk' => 'DH',
			'arhe' => 233,
			'arhs' => 300,
			'arhsk' => 'z',
			'arf' => '黑曲装备F',
			'arfk' => 'DF',
			'arfe' => 233,
			'arfs' => 300,
			'arfsk' => 'z',
			'ara' => '黑曲装备A',
			'arak' => 'DA',
			'arae' => 233,
			'aras' => 300,
			'arask' => 'z',
			'art' => '黑曲装备T',
			'artk' => 'Ah',
			'arte' => 233,
			'arts' => 233,
			'artsk' => 'z',
			),
			3 => array
			(
			'name' => '北京推倒你',
			'gd' => 'm',
			'icon' => 102,
			'club' => 13,
			'clubskill' => Array(
				'buff_duel',
			),
			'clubskillpara' => Array
			(
				'c13_quick' => Array(
					'lvl' => 4,
				),
				'c13_wingchun' => Array(
					'lvl' => 2,
				),
				'c13_duel' => Array(
					'active-t' => 1,
				),
			),
			'skill' => 600,
			'wep' => '拳头',
			'wepk' => 'WN',
			'wepe' => 400,
			'weps' => 998,
			'wepsk' => 'hrden',
			'arb' => 'Microsoft Visual Studio 2010',
			'arbk' => 'DB',
			'arbe' => 2333,
			'arbs' => 300,
			'arbsk' => 'A',
			'arh' => 'GoldWave',
			'arhk' => 'DH',
			'arhe' => 2333,
			'arhs' => 300,
			'arhsk' => 'b',
			'arf' => 'MeGUI',
			'arfk' => 'DF',
			'arfe' => 2333,
			'arfs' => 300,
			'arfsk' => 'a',
			'ara' => 'Micorsoft AppLocale',
			'arak' => 'DA',
			'arae' => 2333,
			'aras' => 300,
			'arask' => 'H',
			'itm1' => '肉○器“北京”型',
			'itmk1' => 'WP',
			'itme1' => 4000,
			'itms1' => '∞',
			'itmsk1' => 'g',
			),
			4 => array
			(
			'name' => 'BorisX',
			'gd' => 'm',
			'icon' => 105,
			'hp' => 4900,
			'club' => 4,
			'clubskillpara' => Array
			(
				'c4_stable' => Array(
					'lvl' => 6,
					'costcount' => 20,
				),
				'c4_break' => Array(
					'lvl' => 3,
					'costcount' => 19,
				),
				'c4_sniper' => Array(
					'active' => 1,
				),
			),
			'skill' => 800,
			'wep' => 'AKM-ExS',
			'wepk' => 'WJ',
			'wepe' => 2000,
			'weps' => 400,
			'wepsk' => 'n',
			),
			5 => array
			(
			'name' => 'Renamon',
			'gd' => 'f',
			'mhp' => 19000,
			'icon' => 98,
			'club' => 2,
			'clubskill' => Array(
				'buff_annihil'
			),
			'clubskillpara' => Array
			(
				'c2_intuit' => Array(
					'lvl' => 6,
				),
			),
			'skill' => 600,
			'wep' => '画(ping)笔(ru)',
			'wepk' => 'WK',
			'wepe' => 300,
			'weps' => 999,
			'wepsk' => 'rdn',
			),
			6 => array
			(
			'name' => 'beijuzhu',
			'gd' => 'm',
			'icon' => 10,
			'club' => 7,
			'clubskill' => Array
			(
				'buff_shield'
			),
			'clubskillpara' => Array
			(
				'c7_shield' => Array(
					'lvl' => 5,
				),
				'buff_shield' => Array(
					'svars' => 1766,
				),
			),
			'skill' => 1000,
			'wep' => '破解的PSP-3000',
			'wepk' => 'WP',
			'wepe' => 400,
			'weps' => 999,
			'wepsk' => 'rne',
			),
			7 => array
			(
			'name' => '捂脸姬',
			'gd' => 'f',
			'pose' => 4,
			'att' => 4000,
			'icon' => 10,
			'inf' => 'p',
			'club' => 8,
			'clubskillpara' => Array
			(
				'c8_expert' => Array(
					'lvl' => 4,
				),
				'c8_infilt' => Array(
					'lvl' => 6,
				),
			),
			'skill' => 600,
			'wep' => '超级☆无敌辰音LILY',
			'wepk' => 'WD',
			'wepe' => 1000,
			'weps' => 999,
			'wepsk' => 'cdn',
			),
			8 => array
			(
			'name' => 'Yoshiko-G',
			'horizon' => 1,
			'gd' => 'f',
			'def' => 30250,
			'icon' => 4,
			'club' => 4,
			'clubskillpara' => Array
			(
				'c4_stable' => Array(
					'lvl' => 6,
					'costcount' => 20,
				),
				'c4_break' => Array(
					'lvl' => 3,
					'costcount' => 19,
				),
				'c4_roar' => Array(
					'active' => 1,
				),
			),
			'skill' => 600,
			'wep' => '光翼型近接支援残酷戦闘機',
			'wepk' => 'WG',
			'wepe' => 5000,
			'weps' => 999,
			'wepsk' => 'nre',
			),
			9 => array
			(
			'name' => 'lemon',
			'horizon' => 1,
			'gd' => 'r',
			'icon' => 0,
			'club' => 98,
			'lvl' => 0,
			'rage' => 0,
			'mhp' => 8400,
			'att' => 32767,
			'def' => 32767,
			'skill' => 1300,
			'wep' => '大纸扇',
			'wepk' => 'WP',
			'wepe' => 4,
			'weps' => 20,
			'wepsk' => 'n',
			'arb' => '工作装',
			'arbk' => 'DB',
			'arbe' => 15,
			'arbs' => 8,
			'arbsk' => 'Hz',
			'arh' => '防灾头巾',
			'arhk' => 'DH',
			'arhe' => 3,
			'arhs' => 10,
			'arhsk' => 'Aa',
			'ara' => '垫肩',
			'arak' => 'DA',
			'arae' => 5,
			'aras' => 3,
			'arask' => 'Bb',
			'arf' => '钉鞋',
			'arfk' => 'DF',
			'arfe' => 10,
			'arfs' => 5,
			'arfsk' => 'cl',
			'art' => 'Untainted Glory',
			'artk' => 'A',
			'arte' => 1,
			'arts' => 1,
			'artsk' => 'h',
			'itm1' => '高级飞镖',
			'itmk1' => 'WC',
			'itme1' => 8,
			'itms1' => 48,
			'itmsk1' => 'n',
			'itm2' => '照明弹',
			'itmk2' => 'WD',
			'itme2' => 20,
			'itms2' => 60,
			'itmsk2' => 'nd',
			'itm3' => '火绳枪',
			'itmk3' => 'WG',
			'itme3' => 30,
			'itms3' => 60,
			'itmsk3' => 'n',
			'itm4' => '叉子',
			'itmk4' => 'WK',
			'itme4' => 10,
			'itms4' => 20,
			'itmsk4' => 'n',
			'itm5' => '退魔符',
			'itmk5' => 'WF',
			'itme5' => 14,
			'itms5' => 60,
			'itmsk5' => 'n',
			'itm6' => 'ACDTS Farming Helper',
			'itmk6' => 'ME',
			'itme6' => 20,
			'itms6' => 20,
			'itmsk6' => 'z',
			),
		),
	),
		22 => array
		(
		'mode' => 1,
		'num' => 2,
		'club' => 0,
		'bid' => 0,
		'inf' => '',
		'rage' => 255,
		'pose'=> 1,
		'tactic' => 3,
		'killnum' => 99,
		'teamID' => '',
		'teamPass' => '',
		'gd' => 'f',
		'pls' => 34,
		'mhp' => 10000000,
		'msp' => 4000,
		'att' => 200000,
		'def' => 4000,
		'lvl' => 97,
		'skill' => 2800,
		'money' => 1,
		'arb' => '天神的荣光',
		'arbk' => 'DB',
		'arbe' => 6000,
		'arbs' => 400,
		'arbsk' => 'Aa',
		'arh' => '天神的荣光',
		'arhk' => 'DH',
		'arhe' => 4000,
		'arhs' => 300,
		'arf' => '天神的荣光',
		'arfk' => 'DF',
		'arfe' => 4000,
		'arfs' => 300,
		'ara' => '天神的荣光',
		'arak' => 'DA',
		'arae' => 4000,
		'aras' => 300,
		'art' => '天神的荣光',
		'artk' => 'A',
		'arte' => 200,
		'arts' => 300,
		'artsk' => 'Hc',
		'sub' => array
		(
			0 => array
			(
			'name' => '冴月麟MK-II',
			'icon' => 3,
			'club' => 98,
			'pose'=> 4,
			'tactic' => 3,
			'wep' => '键 希望弹',
			'wepk' => 'WC',
			'wepe' => 5000,
			'weps' => 998,
			'wepsk' => 'zdfkc',
			'itm1' => '键 燃烧弹',
			'itmk1' => 'WF',
			'itme1' => 4800,
			'itms1' => '∞',
			'itmsk1' => 'Nfzr',
			'itm2' => '键 生命弹',
			'itmk2' => 'WG',
			'itme2' => 5000,
			'itms2' => 998,
			'itmsk2' => 'zdrfk',
			'itm3' => '键 未来弹',
			'itmk3' => 'WP',
			'itme3' => 5000,
			'itms3' => '∞',
			'itmsk3' => 'rcdfN',
			'itm4' => '键 催泪弹',
			'itmk4' => 'WD',
			'itme4' => 5000,
			'itms4' => 998,
			'itmsk4' => 'rkdz',
			'itm4' => '键 旅途弹',
			'itmk4' => 'WK',
			'itme4' => 5000,
			'itms4' => 998,
			'itmsk4' => 'zdNkc',
			'itm5' => '键 审判弹',
			'itmk5' => 'WF',
			'itme5' => 10000,
			'itms5' => '∞',
			'itmsk5' => 'nrfkx',
			),
			1 => array
			(
			'name' => '星莲船四面BOSS',
			'horizon' => 1,
			'gd' => 'm',
			'icon' => 4,
			'club' => 10,
			'clubskill' => Array(
				'c4_sniper',
			),
			'clubskillpara' => Array
			(
				'c4_sniper' => Array(
					'active' => 1,
				),
				'c10_insight' => Array(
					'lvl' => 4,
				),
			),
			'wep' => '205mm绯牡丹加农炮',
			'wepk' => 'WJ',
			'wepe' => 800,
			'weps' => 8000,
			'wepsk' => 'nrd',
			'itm2' => '腿',
			'itmk2' => 'X',
			'itme2' => 1,
			'itms2' => 1,
			'itmsk2' => 'x',
			'itm1' => '四面的○',
			'itmk1' => 'WD',
			'itme1' => 4444,
			'itms1' => 444,
			'itmsk1' => 'rd',
			),
		),
	),
	24 => array
		(
		'mode' => 2,
		'num' => 3,
		'pass' => 'gbauibg2',
		'club' => 0,
		'bid' => 0,
		'inf' => '',
		'rage' => 88,
		'pose'=> 1,
		'tactic' => 3,
		'killnum' => 0,
		'rp' => 1200,
		'teamID' => '',
		'teamPass' => '',
		'gd' => 'r',
		'pls' => 34,
		'mhp' => 6000,
		'msp' => 400,
		'att' => 200,
		'def' => 400,
		'lvl' => 50,
		'skill' => 300,
		'money' => 800,
		'arb' => '巫师袍子',
		'arbk' => 'DB',
		'arbe' => 1800,
		'arbs' => 300,
		'arbsk' => 'A',
		'arh' => '巫师兜帽',
		'arhk' => 'DH',
		'arhe' => 1200,
		'arhs' => 300,
		'arhsk' => 'DF',
		'arf' => '巫师便鞋',
		'arfk' => 'DF',
		'arfe' => 1700,
		'arfs' => 300,
		'arfsk' => 'a',
		'ara' => '巫师手套',
		'arak' => 'DA',
		'arae' => 1900,
		'aras' => 300,
		'arask' => 'H',
		'art' => '巫师之力',
		'artk' => 'AA',
		'arte' => 1,
		'arts' => 33,
		'artsk' => 'c',
//		'itm5' => '好人卡',
//		'itmk5' => 'Y',
//		'itme5' => 1,
//		'itms5' => 88,
		'sub' => array
		(
			0 => array
			(
			'name' => '胶冻',
			'gd' => 'm',
			'icon' => 225,
			'club' => 4,
			'wep' => '■胶冻の巴雷特■',
			'wepk' => 'WJ',
			'wepe' => 333,
			'weps' => 777,
			'wepsk' => '',
			'arb' => '■胶冻的风衣■',
			'arbk' => 'DB',
			'arbe' => 1800,
			'arbs' => 300,
			'arbsk' => 'A',
			'arh' => '■胶冻的兜帽■',
			'arhk' => 'DH',
			'arhe' => 1200,
			'arhs' => 300,
			'arhsk' => 'DF',
			'arf' => '■胶冻的跑鞋■',
			'arfk' => 'DF',
			'arfe' => 1700,
			'arfs' => 300,
			'arfsk' => 'a',
			'ara' => '■胶冻的手套■',
			'arak' => 'DA',
			'arae' => 1900,
			'aras' => 300,
			'arask' => 'H',
			'art' => '■胶冻的吊坠■',
			'artk' => 'AA',
			'arte' => 1,
			'arts' => 33,
			'artsk' => 'c',
			),
			1 => array
			(
			'name' => 'OGAS',
			'gd' => 'm',
			'club' => 4,
			'icon' => 226,
			'wep' => '■便携式火力平台■',
			'wepk' => 'WJ',
			'wepe' => 333,
			'weps' => 777,
			'wepsk' => '',
			'arb' => '■Type3防弹插板■',
			'arbk' => 'DB',
			'arbe' => 1800,
			'arbs' => 300,
			'arbsk' => 'A',
			'arh' => '■VFL 6-24×56光学瞄具■',
			'arhk' => 'DH',
			'arhe' => 1200,
			'arhs' => 300,
			'arhsk' => 'DF',
			'arf' => '■IOP T4外骨骼■',
			'arfk' => 'DF',
			'arfe' => 1700,
			'arfs' => 300,
			'arfsk' => 'a',
			'ara' => '■热光学迷彩披风■',
			'arak' => 'DA',
			'arae' => 1900,
			'aras' => 300,
			'arask' => 'H',
			'art' => '■遗留的武器库■',
			'artk' => 'AA',
			'arte' => 1,
			'arts' => 33,
			'artsk' => 'c',
			),
			2 => array
			(
			'name' => '菌菌子',
			'gd' => 'f',
			'icon' => 227,
			'club' => 9,
			'wep' => '【工作用数位板】',
			'wepk' => 'WF',
			'wepe' => 333,
			'weps' => 777,
			'wepsk' => '',
			'arb' => '【学园针织罩衫】',
			'arbk' => 'DB',
			'arbe' => 1800,
			'arbs' => 300,
			'arbsk' => 'A',
			'arh' => '【鱼形发卡】',
			'arhk' => 'DH',
			'arhe' => 1200,
			'arhs' => 300,
			'arhsk' => 'DF',
			'arf' => '【学园制服鞋】',
			'arfk' => 'DF',
			'arfe' => 1700,
			'arfs' => 300,
			'arfsk' => 'a',
			'ara' => '【工作用数位笔】',
			'arak' => 'DA',
			'arae' => 1900,
			'aras' => 300,
			'arask' => 'H',
			'art' => '【布艺猫咪挂件】',
			'artk' => 'AA',
			'arte' => 1,
			'arts' => 33,
			'artsk' => 'c',
			),
		),
	),
	26 => array
		(
		'mode' => 2,
		'num' => 1,
		'pass' => 'gbauibg2',
		'club' => 0,
		'bid' => 0,
		'inf' => '',
		'rage' => 0,
		'pose'=> 1,
		'tactic' => 3,
		'killnum' => 0,
		'rp' => 60000,
		'teamID' => '',
		'teamPass' => '',
		'gd' => 'r',
		'pls' => 34,
		'mhp' => 8000,
		'msp' => 8000,
		'att' => 10000,
		'def' => 118000,
		'lvl' => 25,
		'skill' => 8000,
		'money' => 1,
		'arb' => '似乎是把殿下的mtkkk扒了穿上身的白色泳衣',
		'arbk' => 'DB',
		'arbe' => 1,
		'arbs' => 50,
		'arbsk' => 'A',
		'arh' => '琪露诺的胖次',
		'arhk' => 'DH',
		'arhe' => 1,
		'arhs' => 50,
		'arhsk' => 'aI',
		'arf' => '你怎么知道我在233服5821局唱了两万六千次歌？',
		'arfk' => 'DF',
		'arfe' => 1,
		'arfs' => 50,
		'arfsk' => 'h',
		'ara' => '印着【驱寒颂歌】的歌词卡模样的杏仁豆腐',
		'arak' => 'DA',
		'arae' => 1,
		'aras' => 50,
		'arask' => 'z',
		'art' => '快！说！小！仓！唯！唱！歌！贼！好！听！',
		'artk' => 'A',
		'arte' => 1,
		'arts' => 1,
		'artsk' => 'H',
		'sub' => array
		(
			0 => array
			(
			'name' => 'mtkkk',
			'gd' => 'f',
			'icon' => 109,
			'club' => 15,
			'wep' => '冰符『Icicle Fall』',
			'wepk' => 'WF',
			'wepe' => 9,
			'weps' => '∞',
			'wepsk' => 'ri',
			'itm1' => '我在翻录像的时候发现以前的我在说蕾薇希雅很色',
			'itmk1' => 'X',
			'itme1' => 1,
			'itms1' => 1,
			'itmsk1' => '',
			'itm2' => '我的下议会用了五秒的时间回想，蕾薇希雅确实色',
			'itmk2' => 'X',
			'itme2' => 1,
			'itms2' => 1,
			'itmsk2' => '',
			'itm3' => '但是，现在的我觉得小草神更色',
			'itmk3' => 'X',
			'itme3' => 1,
			'itms3' => 1,
			'itmsk3' => '',
			'itm4' => '纳西妲真是太对我叉批辣，我好想和小草神■■■🥵🥵🥵',
			'itmk4' => 'X',
			'itme4' => 1,
			'itms4' => 1,
			'itmsk4' => '',
			'itm5' => '但是那个小草支线写的什么勾八，写的人赶紧去死吧',
			'itmk5' => 'X',
			'itme5' => 1,
			'itms5' => 1,
			'itmsk5' => '',
			'itm6' => '■饱含篝酱怨念的音波搅拌特制篝酱■',
			'itmk6' => 'PM2',
			'itme6' => 120,
			'itms6' => 5,
			'itmsk6' => 'V',
			),
		),
	),

	88 => array
		(
		'mode' => 1,
		'num' => 4,
		'pass' => 'bra',
		'club' => 15,
		'bid' => 0,
		'inf' => '',
		'rage' => 188,
		'pose'=> 4,
		'tactic' => 3,
		'killnum' => 88,
		'teamID' => '',
		'teamPass' => '',
		'gd' => 'm',
		'pls' => 32,
		'mhp' => 8888888,
		'msp' => 888,
		'att' => 88888,
		'def' => 88888,
		'lvl' => 88,
		'skill' => 88888,
		'money' => 88888,
		'arb' => '[数据删除]',
		'arbk' => 'DB',
		'arbe' => 8888,
		'arbs' => 8888,
		'arbsk' => 'Aa',
		'arh' => '[数据删除]',
		'arhk' => 'DH',
		'arhe' => 8888,
		'arhs' => 8888,
		'arhsk' => 'Bb',
		'arf' => '[数据删除]',
		'arfk' => 'DF',
		'arfe' => 8888,
		'arfs' => 8888,
		'arfsk' => 'Mm',
		'ara' => '[数据删除]',
		'arak' => 'DA',
		'arae' => 8888,
		'aras' => 8888,
		'arask' => 'Hc',
		'art' => '[数据删除]',
		'artk' => 'A',
		'arte' => 1,
		'arts' => 1,
		'artsk' => 'xHh',
		'sub' => array
		(
			0 => array
			(
			'name' => 'SCP-682',
			'icon' => 103,
			'wep' => '[数据删除]',
			'wepk' => 'WN',
			'wepe' => 8888,
			'weps' => 8888,
			'wepsk' => 'nrd',
			),
			1 => array
			(
			'name' => 'SCP-173',
			'horizon' => 1,
			'icon' => 106,
			'wep' => '[数据删除]',
			'wepk' => 'WF',
			'wepe' => 8888,
			'weps' => 8888,
			'wepsk' => 'nrd',
			),
			2 => array
			(
			'name' => 'SCP-076',
			'horizon' => 1,
			'icon' => 107,
			'wep' => '[数据删除]',
			'wepk' => 'WK',
			'wepe' => 8888,
			'weps' => 8888,
			'wepsk' => 'nrd',
			),
			3 => array
			(
			'name' => 'SCP-958',
			'icon' => 108,
			'wep' => '[数据删除]',
			'wepk' => 'WG',
			'wepe' => 8888,
			'weps' => 8888,
			'wepsk' => 'nrd',
			),
		),
	),

	90 => array
		(
		'mode' => 2,
		'num' => 280,
		'pass' => 'bra',
		'club' => 0,
		'bid' => 0,
		'inf' => '',
		'rage' => 0,
		'pose'=> 0,
		'tactic' => 0,
		'killnum' => 0,
		'teamID' => '',
		'teamPass' => '',
		'gd' => 'r',
		'pls' => 99,
		'mhp' => 550,
		'msp' => 200,
		'att' => 120,
		'def' => 160,
		'lvl' => 30,
		'skill' => 42,
		'money' => 380,
		'arb' => '西装',
		'arbk' => 'DB',
		'arbe' => 35,
		'arbs' => 30,

		'arh' => '墨镜',
		'arhk' => 'DH',
		'arhe' => 25,
		'arhs' => 30,

		'arf' => '皮鞋',
		'arfk' => 'DF',
		'arfe' => 25,
		'arfs' => 30,

		'ara' => '手套',
		'arak' => 'DA',
		'arae' => 25,
		'aras' => 30,

		'art' => 'ACFUN的账号',
		'artk' => 'A',
		'arte' => 20,
		'arts' => 10,
		'itm4' => '锅？',
		'itmk4' => 'Y',
		'itme4' => 1,
		'itms4' => 1,
		'itm6' => '压缩饼干',
		'itmk6' => 'HB',
		'itme6' => 35,
		'itms6' => 10,
//		'itm5' => '好人卡',
//		'itmk5' => 'Y',
//		'itme5' => 1,
//		'itms5' => 15,

		'sub' => array
		(
		 0 => array
			(
			'name' => 'AC专业喷子',
			'icon' => 21,
			'wep' => '嘴炮',
			'wepk' => 'WG',
			'wepe' => 50,
			'weps' => 50,
			'itm5' => '解毒剂',
			'itmk5' => 'Cp',
			'itme5' => 1,
			'itms5' => 1,
			),
		1 => array
			(
			'name' => 'AC搬运职人',
			'icon' => 22,
			'wep' => '搬运之力',
			'wepk' => 'WP',
			'wepe' => 50,
			'weps' => 50,
			'itm5' => '烧伤药剂',
			'itmk5' => 'Cu',
			'itme5' => 1,
			'itms5' => 1,
			),
		2 => array
			(
			'name' => 'AC翻唱职人',
			'icon' => 24,
			'wep' => '销魂之歌',
			'wepk' => 'WK',
			'wepe' => 50,
			'weps' => 50,
			'itm5' => '解冻药水',
			'itmk5' => 'Ci',
			'itme5' => 1,
			'itms5' => 1,
			),
		3 => array
			(
			'name' => 'AC字幕职人',
			'icon' => 23,
			'wep' => '神字幕',
			'wepk' => 'WC',
			'wepe' => 50,
			'weps' => 50,
			'itm5' => '麻痹药剂',
			'itmk5' => 'Ce',
			'itme5' => 1,
			'itms5' => 1,
			),
		),
	),
	91 => array
		(
		'mode' => 2,
		'num' => 1,
		'pass' => 'bra',
		'club' => 0,
		'bid' => 0,
		'inf' => '',
		'rage' => 0,
		'pose'=> 0,
		'tactic' => 0,
		'killnum' => 0,
		'teamID' => '',
		'teamPass' => '',
		'gd' => 'f',
		'pls' => 99,
		'mhp' => 500,
		'msp' => 200,
		'att' => 130,
		'def' => 150,
		'lvl' => 10,
		'skill' => 40,
		'money' => 220,
		'arb' => '西装',
		'arbk' => 'DB',
		'arbe' => 35,
		'arbs' => 30,

		'arh' => '墨镜',
		'arhk' => 'DH',
		'arhe' => 25,
		'arhs' => 30,

		'arf' => '皮鞋',
		'arfk' => 'DF',
		'arfe' => 25,
		'arfs' => 30,

		'ara' => '手套',
		'arak' => 'DA',
		'arae' => 25,
		'aras' => 30,

		'art' => 'ACFUN的账号',
		'artk' => 'A',
		'arte' => 20,
		'arts' => 10,
		'itm4' => '锅？',
		'itmk4' => 'Y',
		'itme4' => 1,
		'itms4' => 1,
		'itm6' => '压缩饼干',
		'itmk6' => 'HB',
		'itme6' => 35,
		'itms6' => 10,
//		'itm5' => '好人卡',
//		'itmk5' => 'Y',
//		'itme5' => 1,
//		'itms5' => 15,

		'sub' => array
		(
		 0 => array
			(
			'name' => 'AC专业职人',
			'icon' => 21,
			'wep' => '嘴炮',
			'wepk' => 'WG',
			'wepe' => 50,
			'weps' => 50,
			'itm5' => '解毒剂',
			'itmk5' => 'Cp',
			'itme5' => 1,
			'itms5' => 1,
			),
		),
	),

	92 => array
	(
	'mode' => 1,
	'num' => 150,
	'pass' => 'bra',
	'club' => 0,
	'bid' => 0,
	'inf' => '',
	'rage' => 13,
	'pose'=> 7,
	'tactic' => 2,
	'horizon' => 1,
	'killnum' => 0,
	'teamID' => '',
	'teamPass' => '',
	'gd' => 'f',
	'clubskill' => Array(
		'fireseed3',
		'npc_purity',
	),
	//'pls' => 99,
	'mhp' => 11,
	'msp' => 600,
	'att' => 19,
	'def' => 99,
	'lvl' => 17,
	'skill' => 1,
	'money' => 250,
	'arb' => '◆篝火',
	'arbk' => 'DB',
	'arbe' => 1,
	'arbs' => 1,
	'arbsk' => 'z',
	'arh' => '◆埋火',
	'arhk' => 'DH',
	'arhe' => 1,
	'arhs' => 1,
	'arhsk' => 'z',
	'arf' => '◆永火',
	'arfk' => 'DF',
	'arfe' => 1,
	'arfs' => 1,
	'arfsk' => 'z',
	'ara' => '◆残火',
	'arak' => 'DA',
	'arae' => 1,
	'aras' => 1,
	'arask' => 'z',
	'art' => '◆焰火',
	'artk' => 'A',
	'arte' => 1,
	'arts' => 1,
	'artsk' => 'z',
	'itm6' => '◆火之碎片',
	'itmk6' => 'HB',
	'itme6' => 13337,
	'itms6' => 1,

	'sub' => array
	(
	 0 => array
		(
		'name' => '✦覆唱的篝火',
		'icon' => 34,
		'num' => 30,
		'pls' => Array(2,15),
		'wep' => '✧覆唱之歌',
		'wepk' => 'WF',
		'wepe' => 1,
		'weps' => 50,
		'itm5' => '◆愈合数据',
		'itmk5' => '🎆H',
		'itme5' => 5,
		'itms5' => '∞',
		),
	1 => array
		(
		'name' => '✦爱恋的埋火',
		'icon' => 35,
		'num' => 30,
		'pls' => Array(3,22),
		'wep' => '✧无畏之爱',
		'wepk' => 'WF',
		'wepe' => 1,
		'weps' => 50,
		'itm5' => '◆熟练数据',
		'itmk5' => '🎆V',
		'itme5' => 5,
		'itms5' => 1,
		),
	2 => array
		(
		'name' => '✦怜悯的永火',
		'icon' => 36,
		'num' => 30,
		'pls' => Array(18,23),
		'wep' => '✧怜悯之痛',
		'wepk' => 'WF',
		'wepe' => 1,
		'weps' => 50,
		'itm5' => '◆生命数据',
		'itmk5' => '🎆O',
		'itme5' => 10,
		'itms5' => 1,
		),
	3 => array
		(
		'name' => '✦执念的残火',
		'icon' => 37,
		'num' => 30,
		'pls' => Array(20,24),
		'wep' => '✧执念之刺',
		'wepk' => 'WF',
		'wepe' => 1,
		'weps' => 50,
		'itm5' => '◆意念数据',
		'itmk5' => '🎆D',
		'itme5' => 10,
		'itms5' => 1,
		),
	4 => array
		(
		'name' => '✦希望的焰火',
		'icon' => 38,
		'num' => 30,
		'pls' => Array(12,29),
		'wep' => '✧希望之风',
		'wepk' => 'WF',
		'wepe' => 1,
		'weps' => 50,
		'itm5' => '✦ЦВЙΨХЩΗЖФ✦',
		'itmk5' => '🎆B',
		'itme5' => 1,
		'itms5' => 1,
		'itmsk5' => '',
		),
	),
),

	// 93 = RAID容器；由规则逻辑随机放置在安全地点，且不会主动攻击。
	// 93 = RAID containers; behavior places them at safe locations and they never attack proactively.
	93 => array
	(
		'mode' => 1,
		'num' => 48,
		'pass' => 'bra',
		'club' => 0,
		'bid' => 0,
		'inf' => '',
		'rage' => 0,
		'pose' => 0,
		'tactic' => 2,
		'killnum' => 0,
		'teamID' => 'RAID_CONTAINER',
		'teamPass' => 'RAID_CONTAINER',
		'gd' => 'n',
		'pls' => 99,
		'mhp' => 600,
		'msp' => 1,
		'att' => 1,
		'def' => 35,
		'lvl' => 8,
		'skill' => 1,
		'money' => 0,
		'clbpara' => Array(
			'raid' => Array(
				'role' => 'container',
				'immune_to_wipe' => true,
			),
		),
		'wep' => '松脱的铰链',
		'wepk' => 'WP',
		'wepe' => 1,
		'weps' => 9999,
		'wepsk' => '',
		'sub' => array
		(
			0 => array('name' => '简易物资箱', 'icon' => 202, 'mhp' => 450, 'def' => 20),
			1 => array('name' => '加固军需箱', 'icon' => 202, 'mhp' => 750, 'def' => 55),
			2 => array('name' => '加密样本柜', 'icon' => 202, 'mhp' => 950, 'def' => 80),
			3 => array('name' => '异常封存舱', 'icon' => 202, 'mhp' => 1300, 'def' => 120),
		),
	),
);

$npcdescription = array
(
	1 => array
	(
		'sub' => array
		(
			0 => array
			(
				'count' => 1,
				'description' => '红杀将军 红暮，坐镇于无月之影的BOSS。<span class="yellow">本RAID中旧锁定解除路线已经失效，没事还是不要惹她好了……</span>',
			),
		),
	),
	4 => array
	(
		'sub' => array
		(
			0 => array
			(
				'count' => 0,
				'description' => '拟似意识 ■ 的历史资料。<span class="yellow">本RAID已移除破灭之诗的取得路线，常规手段无法召唤该个体。</span>',
			),
		),
	),
	5 => array
	(
		'sub' => array
		(
			0 => array
			(
				'count' => 1,
				'description' => '杏仁豆腐 冴月 麟，对玩家有较大威胁，掉落启动死斗模式的钥匙，和很强力的灵系武器。',
			),
			1 => array
			(
				'count' => 1,
				'description' => '杏仁豆腐 某四面，对玩家有较大威胁，掉落启动死斗模式的钥匙，和很强力的射系武器。',
			),
		),
	),
	6 => array
	(
		'sub' => array
		(
			0 => array
			(
				'count' => 1,
				'description' => '黑幕 Acg_Xilin，对玩家具有很大威胁，如果防御值没有达到2000最好不要惹他（她）。掉落可以杀死任意一名玩家的道具“死亡笔记”，和大量的金钱。',
			),
		),
	),
	9 => array
	(
		'sub' => array
		(
			0 => array
			(
				'count' => 1,
				'description' => '这是红暮身边的奇妙的幻影。<span class="yellow">感觉红暮很寂寞的样子……</span>',
			),
		),
	),
	11 => array
	(
		'sub' => array
		(
			0 => array
			(
				'count' => 0,
				'description' => '真职人 Hank，使用带有全属性的爆系武器，掉落极优秀的防具。',
			),
			1 => array
			(
				'count' => 0,
				'description' => '真职人 爱情上甘岭，使用带有全属性的斩系武器，掉落极优秀的防具。',
			),
			2 => array
			(
				'count' => 0,
				'description' => '真职人 221，使用带有全属性的殴系武器，掉落极优秀的防具。',
			),
			3 => array
			(
				'count' => 0,
				'description' => '真职人 Erul Tron，使用带有全属性的射系武器，掉落极优秀的防具。',
			),
			4 => array
			(
				'count' => 0,
				'description' => '真职人 【77-JACKPOT】，使用带有全属性的殴系武器，掉落极优秀的防具。',
			),
			5 => array
			(
				'count' => 0,
				'description' => '真职人 便当盒，ACFUN大逃杀吉祥物，卖萌的存在，拥有888万点HP和双抹消饰品，几乎无法杀死。',
			),
		),
	),
	13 => array
	(
		'sub' => array
		(
			0 => array
			(
				'count' => 1,
				'description' => '循环使者 妖精幻象-麦基，<span class="yellow">携带强力的骰子道具［Ｄ１０］，可以为你产生高级空想道具。</span>',
			),
			1 => array
			(
				'count' => 1,
				'description' => '循环使者 妖精幻象-拉普尼，<span class="yellow">携带强力的骰子道具［Ｄ４０］，可以从地图上随机获得物品。</span>',
			),
			2 => array
			(
				'count' => 1,
				'description' => '循环使者 妖精幻象-斯洛蒂，<span class="yellow">携带强力的骰子道具［Ｄ１００］，可以重新投掷你的各色数值。</span>',
			),
		),
	),
	14 => array
	(
		'sub' => array
		(
			0 => array
			(
				'count' => 1,
				'description' => '数据碎片 讲解员 梦美：<span class="yellow">被击杀后会变身为攻击力极强的NPC</span>。',
			),
			1 => array
			(
				'count' => 1,
				'description' => '数据碎片 喧哗少女 叶留佳：<span class="yellow">被击杀后会变身为攻击力极强的NPC</span>。',
			),
			2 => array
			(
				'count' => 1,
				'description' => '数据碎片 风纪委员 静流：<span class="yellow">被击杀后会变身为攻击力极强的NPC</span>。',
			),
			'讲解员 梦美' => array
			(
				'count' => 0,
				'description' => '数据碎片 战斗模式 梦美：讲解员 梦美的第二形态，装备反装甲武器、能够镇压任何失控机械的『本能模式』。<span class="yellow">实力在数据碎片中属于中等。</span>',
			),
			'喧哗少女 叶留佳' => array
			(
				'count' => 0,
				'description' => '数据碎片 本气（？） 叶留佳：喧哗少女 叶留佳的第二形态。这个小叶子是吃错了什么药吗？？<span class="yellow">在数据碎片中攻击面最弱，防御面最强，如果你的武器足够强力的话，可以试试优先挑战她。</span>',
			),
			'风纪委员 静流' => array
			(
				'count' => 0,
				'description' => '数据碎片 守卫者 静流：风纪委员 静流的第二形态，以飒爽的战斗姿态出现的强大超能力者。<span class="yellow">在数据碎片中攻击面最强，防御面最弱，虽然一推就倒但也让无数玩家阴沟里翻船。</span><br>不知为何总给人一种摸不着头脑的感觉……',
			),
		),
	),
	15 => array
	(
		'sub' => array
		(
			0 => array
			(
				'count' => 0,
				'description' => '抹杀使徒 【SANMA_TK】，似乎很喜欢吃秋刀鱼罐头，沙包的守护神，触手的噩梦。<span class="yellow">熟练度50是诈称，实际上是殴熟300、斩熟555、射熟880、投熟684。</span>她出场后，玩家还可以在聊天框问她KEY系半身像和小黄精灵球的部件在哪里喔。',
			),
		),
	),
	19 => array
	(
		'sub' => array
		(
			0 => array
			(
				'count' => 0,
				'description' => '接受玩家的挑战，展现出作为红杀领导人五成功力的红暮。<span class="yellow">请注意：她身穿的战甲可以进行多种多样的攻击。</span>战甲上的武器在没有作为主武器使用时，都带有产生防护罩的功能。',
			),
			1 => array
			(
				'count' => 0,
				'description' => '和红暮并肩作战，货真价实的蓝凝。<span class="yellow">请注意：她似乎换了个武器，可以在场地上设下延时伤害的陷阱。</span>运气不好的话，就连战场上其他人都会被卷入的。',
			),
		),
	),
	24 => array
	(
		'sub' => array
		(
			0 => array
			(
				'count' => 1,
				'description' => '负责游戏宣传，文案以及其他相关内容。',
			),
			1 => array
			(
				'count' => 1,
				'description' => '负责游戏测试，联系人员，文案以及其他相关内容。',
			),
			2 => array
			(
				'count' => 1,
				'description' => '负责部分剧情NPC的图像人设，以及全体NPC的Q版人设。',
			),
		),
	),
	21 => array
	(
		'sub' => array
		(
			0 => array
			(
				'count' => 1,
				'description' => '武神 虚子，<span class="yellow">英灵四大天王之首，被称为英灵殿唯一的良心。（咦？）</span><br><span class="red">小心直死啊！！</span>',
			),
			1 => array
			(
				'count' => 1,
				'description' => '武神 水月，<span class="yellow">英灵四大天王之一。</span>身上的火金符对玩家威胁很大，称号为换装迷宫，台词来源于圣经。初始武器自带双抹，但威力不足。',
			),
			2 => array
			(
				'count' => 1,
				'description' => '武神 黑色奪魂曲，<span class="yellow">英灵殿吉祥物。</span>攻击力低，二形态血量为1000W，双抹消，危險性不會很高。',
			),
			3 => array
			(
				'count' => 1,
				'description' => '武神 北京推倒你，<span class="yellow">普通武神。</span>武器为空手，危險性不算高。',
			),
			4 => array
			(
				'count' => 1,
				'description' => '武神 BorisX，<span class="yellow">英灵四大天王之一。</span>武器为重枪，<span class="red">小心貫穿+重擊！</span>',
			),
			5 => array
			(
				'count' => 1,
				'description' => '武神 Renamon，<span class="yellow">普通武神。</span>血量为武神中最高，但危險性也不算高。',
            ),
			6 => array
			(
				'count' => 1,
				'description' => '武神 beijuzhu，<span class="yellow">普通武神。</span>危險性中等。',
            ),
			7 => array
			(
				'count' => 1,
				'description' => '武神 捂脸姬，<span class="yellow">普通武神。</span>少数没有强袭姿态的武神之一，危險性較低。',
            ),
			8 => array
			(
				'count' => 1,
				'description' => '武神 Yoshiko-G，<span class="yellow">普通武神。</span>危險性不算高。',
            ),
			9 => array
			(
				'count' => 1,
				'description' => '武神 lemon，<span class="yellow">英灵四大天王之一。</span><span class="yellow">拥有最高的基础攻击与防御，十分吓人。</span>性別因為是隨機，是迷惑又或是激怒呢，隨緣吧。',
			),
		),
	),
	22 => array
	(
		'sub' => array
		(
			0 => array
			(
				'count' => 1,
				'description' => '天神 冴月麟MK-II，ACFUN大逃杀的创作者。KEY社的铁杆粉丝。会切换武器。（没有键 催泪弹是刻意的）',
			),
			1 => array
			(
				'count' => 1,
				'description' => '天神 星莲船四面BOSS，AC大逃杀程序员、抖M。虽然武器的攻击力不高，但由于重型枪械的百分比伤害设定，实际上对玩家拥有即死效果。',
			),
		),
	),
	88 => array
	(
		'sub' => array
		(
			0 => array
			(
				'count' => 1,
				'description' => 'SCP-682，坐镇于SCP研究设施，完全无敌的存在，要是不幸被打到的话，必死无疑。最好的避免方法，就是<span class="yellow">永远不要去SCP研究设施</span>。',
			),
			1 => array
			(
				'count' => 1,
				'description' => 'SCP-173，坐镇于SCP研究设施，完全无敌的存在，要是不幸被打到的话，必死无疑。最好的避免方法，就是<span class="yellow">永远不要去SCP研究设施</span>。',
			),
			2 => array
			(
				'count' => 1,
				'description' => 'SCP-076，坐镇于SCP研究设施，完全无敌的存在，要是不幸被打到的话，必死无疑。最好的避免方法，就是<span class="yellow">永远不要去SCP研究设施</span>。',
			),
			3 => array
			(
				'count' => 1,
				'description' => 'SCP-958，坐镇于SCP研究设施，完全无敌的存在，要是不幸被打到的话，必死无疑。最好的避免方法，就是<span class="yellow">永远不要去SCP研究设施</span>。',
			),
		),
	),
	89 => array
	(
		'sub' => array
		(
			0 => array
			(
				'count' => 0,
				'description' => '变幻自如的电子狐狸，眼睛里面似乎内置了一个扫描仪……',
			),
			1 => array
			(
				'count' => 0,
				'description' => '可爱活泼的猫咪幽灵，俗话说猫有九命，但这位猫娘似乎有着更多的残机数量……',
			),
			2 => array
			(
				'count' => 0,
				'description' => '被沉重锁链镣铐紧缚的翼人，不过似乎如果你愿意发善心，使用自己的血肉来打破枷锁的话……',
			),
			3 => array
			(
				'count' => 0,
				'description' => '虽然是害羞的少女，但是拳头似乎很硬，可以打碎一切来犯之敌……',
			),
			4 => array
			(
				'count' => 0,
				'description' => '私自黑入虚拟幻境的黑客，她没有敌意，希望你同等对待她，否则……',
			),
			5 => array
			(
				'count' => 0,
				'description' => '不知道是妖精还是蜜蜂的玩意，因为身形很小，可能打不出某个数值以上的伤害吧——不过……',
			),
			6 => array
			(
				'count' => 0,
				'description' => '没有反应，似乎就是个特别能抗伤害的种火。但真的是这样吗？',
			),
		),
	),
	90 => array
	(
		'sub' => array
		(
			0 => array
			(
				'count' => 75,
				'description' => '无威胁的小兵，也是数目最多的NPC。neta自ACFUN上那些各类不气死人不罢休的喷子，碰见了就上去给他一击吧。',
			),
			1 => array
			(
				'count' => 75,
				'description' => '无威胁的小兵，也是数目最多的NPC。neta自ACFUN上那些搬运nico视频的职人，碰见了就上去给他一击吧。',
			),
			2 => array
			(
				'count' => 75,
				'description' => '无威胁的小兵，也是数目最多的NPC。neta自ACFUN上那些翻唱名曲毁耳不倦的职人，碰见了就上去给他一击吧。',
			),
			3 => array
			(
				'count' => 75,
				'description' => '无威胁的小兵，也是数目最多的NPC。neta自ACFUN上那些弹幕当积木摆的职人，碰见了就上去给他一击吧。',
			),
		),
	),
	92 => array
	(
		'sub' => array
		(
			0 => array
			(
				'count' => 30,
				'description' => '最近出现在虚拟幻境中的迷之存在。将会掉落可强化的永久补给。虽然升级前用起来很麻烦，升级本身也很麻烦，但总比没有好。',
			),
			1 => array
			(
				'count' => 30,
				'description' => '最近出现在虚拟幻境中的迷之存在。将会掉落可强化的全系熟练加强物品。虽然升级前用起来很麻烦，升级本身也很麻烦，但总比没有好。',
			),
			2 => array
			(
				'count' => 30,
				'description' => '最近出现在虚拟幻境中的迷之存在。将会掉落可强化的生命值上限增加物品。虽然升级前用起来很麻烦，升级本身也很麻烦，要切换成体力上限增加物品也很麻烦，但总比没有好。',
			),
			3 => array
			(
				'count' => 30,
				'description' => '最近出现在虚拟幻境中的迷之存在。将会掉落可强化的防御力增加物品。虽然升级前用起来很麻烦，升级本身也很麻烦，要切换成攻击力增加物品也很麻烦，但总比没有好。',
			),
			4 => array
			(
				'count' => 30,
				'description' => '最近出现在虚拟幻境中的迷之存在。将会掉落银白盒子，虽然用起来很麻烦，但总比没有好。',
			),
			//10 => array
			//(
			//	'count' => 0,
			//	'description' => '→检测到入侵行为！←',
			//),
		),
	),
);

// 容器总量读取RAID中央配置，保留48作为旧缓存环境的后备值。
// Read container quantity from central RAID config, retaining 48 for legacy cache contexts.
$raid_container_cfg = function_exists('raid_get_config') ? raid_get_config('containers') : Array();
$npcinfo[93]['num'] = max(0, intval(isset($raid_container_cfg['count']) ? $raid_container_cfg['count'] : 48));
unset($raid_container_cfg);

// RAID容器图鉴 / RAID container encyclopedia entries
$npcdescription[93] = array(
	'sub' => array(
		0 => array('count' => 12, 'description' => '封条松动的基础容器；击破后会生成可收纳战利品。'),
		1 => array('count' => 12, 'description' => '经过装甲加固的军需容器，耐久更高。'),
		2 => array('count' => 12, 'description' => '带有异常加密标记的样本柜，可能藏有高价值宝藏。'),
		3 => array('count' => 12, 'description' => '危险等级最高的异常封存舱，可能藏有特殊宝藏。'),
	),
);

?>
